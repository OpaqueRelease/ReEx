...
def train_and_explain(X, Y, subset = 100):
                    ...
                    for item2 in range(setLength):
                        if item1 != item2:
                            ...
                            if ancestor_element is not None:
                                ...
                                if generalizationDepth * depthWeight == 0 or intersectionRatio/(generalizationDepth * depthWeight) < 0.5:
...
def print_results(class_names, subsets, evaluation):
    ...
    for i in range(len(class_names)):
        output_list = ["RESULT_TAG", str(class_names[i]), str(subsets[0][i]), str(evaluation[i]), str(subsets[1])]
...
def generalizeAncestor(ontology_graph, explanations = None, attributes = None, target_relations = {"is_a","partOf"}, test_run = False, depthWeight = 0):
    ...
    if test_run:
        ...
        subsets = ancestor_multiple_sets([t1,t2], ontology_subgraph)
        evaluation = evaluate([t1,t2], subsets[0])
        print_results(["t1", "t2"], subsets, evaluation, depthWeight = depthWeight)
        ...
        for class_name, explanation_vector in explanations.items():
            ...
            max = np.amax(greater_than_zero_vector)
            print("max: ", max)
            threshold = 0.9 * max
            while True:
                above_threshold = set(np.argwhere(greater_than_zero_vector >= threshold).flatten())
               
                if len(above_threshold) > 5 or threshold < 0.3 * max:
                    break
                threshold *= 0.9
        ...
        print(class_names)
        print("before generalization:", term_sets_per_class)
        print_results(class_names, subsets, evaluation)    
    
    #for a_set in subsets:
    #    ancestor_set = list_to_ancestor_set(ontology_graph, a_set)
    #    ancestor_set = ancestor_set.union(a_set)
    #    G = ontology_graph.subgraph(ancestor_set)
    #    nx.draw(G)
    #    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--depth_weight",default=0.5, type = float)
    ...
    explanations, attributes = train_and_explain(parsed_dataset, target_vector)

    # for k,v in explanations.items():
    #     print(k,v)
        
    generalizeAncestor(ontology_graph, explanations = explanations, attributes = attributes, test_run = False, depthWeight = args.depth_weight)
    
    #generalizeAncestor(ontology_graph, test_run = True)
